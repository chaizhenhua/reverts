...
1. **锚定 anchor**:把内联岛代码匹配到真实包 `(name, version)`。确定性指纹匹配
   (`match`),或对无 node_modules 身份的库由 agent 提议
   (`package candidates`)+ 指纹确认。
...
- **风险**:高 —— 需 fresh re-import + match(在已匹配 DB 上重跑会
...
绝不在 `project.sqlite` 上重跑 `match`(会重复 synthetic sources →
...
2. `/tmp/gen_import_evidence.py` → `import` → `classify --auto --apply`
   → `match --package-source-root … --apply`(**含本期的锚定精度/覆盖改进**)。
3. `generate --source-root src`。

**命名迁移**:`name clusters` 是 **fingerprint-keyed**,内容不变则 fingerprint 不变 →
已命名的 121 个 cluster 在 fresh DB 上**仍适用**;只需把 `island_cluster_names` 行从旧 DB
迁到 fresh DB(`SELECT … ` + `name clusters --batch`)。同理 `module_path_overrides` /
...
实测:`package candidates --accept zod` + `match --package-name zod --apply`
...
一次性提议,跑一次**整夜批量** `match`,再外化 —— 但单次可能数小时。
