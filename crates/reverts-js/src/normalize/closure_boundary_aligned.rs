...
/// `return (() => { ... })();` with the arrow's inner statements, aligning the
/// closure boundary with non-IIFE definitions. Expression-form arrows
/// (`(() => expr)()`) are converted to `return expr;`.
///
/// Only zero-argument, non-async IIFEs are inlined; anything else is left
/// untouched to avoid changing observable semantics.
