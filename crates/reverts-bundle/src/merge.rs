use reverts_input::{ModuleInput, SourceFileInput, SourceSpan};
...
/// Prefix marking an in-memory synthetic source file — content generated by
/// the extractor (reconstructed esbuild multi-handle modules) rather than read
/// from disk. `reverts_input` skips on-disk loading for these paths.
pub const SYNTHETIC_SOURCE_PREFIX: &str = "__reverts_synthetic__/";
...
pub struct MergeOutput {
    ...
    /// (esbuild multi-handle handles). Must be appended to the bundle's
    /// `source_files` before the corresponding `new_modules`.
...
pub fn merge_classification(
    ...
    next_synthetic_source_file_id: u32,
) -> MergeOutput {
    let mut updated = Vec::new();
    let mut new_modules = Vec::new();
    let mut new_source_files = Vec::new();
    let mut next_sf_id = next_synthetic_source_file_id;
...
fn push_new_from_inner(
    ...
    source_file_id: u32,
    synthetic_id: u32,
    next_synthetic_source_file_id: &mut u32,
    new_source_files: &mut Vec<SourceFileInput>,
    ...
    // Synthetic (reconstructed) modules carry their own generated source. Back
    // them with a fresh in-memory source file and span the whole text, rather
    // than a slice of the parent bundle.
    let (backing_source_file_id, source_span) = if let Some(text) = &inner.synthetic_source {
        let sf_id = *next_synthetic_source_file_id;
        *next_synthetic_source_file_id += 1;
        let span = SourceSpan {
            byte_start: 0,
    ...
    let mut row = ModuleInput {
        ...
        original_name,
        semantic_path,
        source_file_id: Some(backing_source_file_id),
...
mod tests {
    ...
    fn overlap_replaces_span_and_preserves_upstream_metadata() {
        ...
        let result = merge_classification(1, &upstream, &classification, 1000, 9000);
    ...
    fn inner_with_no_upstream_becomes_new_module() {
        ...
        let result = merge_classification(1, &upstream, &classification, 5000, 9000);
        assert_eq!(result.new_modules.len(), 1);
    ...
    #[test]
    fn synthetic_inner_becomes_module_backed_by_new_source_file() {
        // A reconstructed multi-handle handle carries synthetic source; merge
        // must back it with a fresh in-memory source file and point the new
        // module's span at the whole synthetic text (not the parent bundle).
        let synthetic = "var a, X=st(()=>{a=1});";
        let inner = InnerModule {
            virtual_id: "esbuild:X".into(),
            body_span: ByteRange::new(0, 0),
            bundler: BundlerKind::Esbuild,
            source_path_hint: None,
            parent_module_id: ModuleId(0),
            synthetic_source: Some(synthetic.to_string()),
        };
        let classification = BundleClassification::Marked(MarkedMetadata {
            inner_modules: vec![inner],
            detected_by: BundlerKind::Esbuild,
        });
        let result = merge_classification(1, &[], &classification, 5000, 9000);

        assert_eq!(result.new_modules.len(), 1);
        assert_eq!(result.new_source_files.len(), 1);
    ...
    fn plain_classification_passes_upstream_through() {
        ...
        let result = merge_classification(1, &upstream, &BundleClassification::Plain, 1000, 9000);
