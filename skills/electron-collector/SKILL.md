...
2. Run `reverts-cli import --input <Contents/Resources/app>
...
`import` always discovers modules; there is no inventory-only import
...
persisted SQLite project/source-unit/module counts (via `report inventory`)
...
**Stage them at generate time** with `--shell-resources`, so the output is
self-contained:

```bash
reverts-cli generate --input <db> --project-id <id> --output <dir> \
  --source-root src --shell-resources /path/to/App.app
```

This copies the dock icon (`electron.icns`), tray images, the app's i18n message
catalogs (`i18n/`), and platform locale bundles (`*.lproj`, `locales/`) from the
source `.app` (or its `Contents/Resources`) into `<dir>/resources/`. Without it the
packaged app shows Electron's default icon and the tray menu hits missing
translations (`formatIntlMessage`). Then, because the dock icon is bundle-driven,
package the output into a branded `.app`
(`CFBundleName`, `CFBundleIconFile=electron` → the staged `electron.icns`) — see
[`decompile-eval/gui-boot-smoke/brand-launch.sh`](../../decompile-eval/gui-boot-smoke/brand-launch.sh).
...
6. Only after mechanical recovery is structurally valid, use the semantic-name
   commands (`name symbols`, `name bindings`, `name plan`) for Agent naming.
...
| `import` rejects manifest | `reverts-cli` exits non-zero with a manifest coverage/evidence error | Fix the collector or ReverTS import mechanism with a regression test; do NOT mutate JSON to satisfy the validator |
| `import` fails during source preparation or discovery | `reverts-cli` error names source-unit IDs and parse/format reason | Reproduce with the named source, add a ReverTS regression test, fix the upstream parser/metadata/discovery path, regenerate the manifest, and import again |
| `import` runs long during discovery | SQLite source-unit/module counts are increasing or logs show variable-flow/signature work | Let the command finish; on interruption, inspect persisted counts with `report inventory` and continue from the existing DB rather than re-importing blindly unless no project row was created |
| Source-unit count mismatch after import | `report inventory` source-unit count != `manifest.sources.length` | Stop; file as ReverTS bug. Re-importing will mask the defect |
...
<!-- TODO(reverts-cli): manifest shape reconciliation — `import
...
The `report inventory --manifest` flag likewise expects a
