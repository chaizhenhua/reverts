...
|---|---|---|
| Project metadata, decompile status, coverage, regeneration | `reverts-cli` (`naming-progress --input <db> --project-id <id> [--json]` and `coverage-ledger --input <db> --project-id <id>` for status/coverage; `generate --input <db> --project-id <id> --output <dir> --source-root src` to regenerate after a pipeline fix; `runtime-inventory --input <db> --project-id <id>` for emitted runtime helpers; `module-classify --input <db> --project-id <id> --batch <TSV> --apply` to persist classifications) <!-- TODO(reverts-cli): list_app_artifacts / get_artifact_manifest / query have no direct CLI equivalent --> | Use DB/AST-backed data. Do not infer runtime profile from filenames alone when artifact metadata exists. |
