...
|---|---|---|
| Project metadata, decompile status, coverage, regeneration | `reverts-cli` (`name progress --input <db> --project-id <id> [--json]` and `report coverage --input <db> --project-id <id>` for status/coverage; `generate --input <db> --project-id <id> --output <dir> --source-root src` to regenerate after a pipeline fix; `report runtime --input <db> --project-id <id>` for emitted runtime helpers) <!-- TODO(reverts-cli): list_app_artifacts / get_artifact_manifest / query / update_modules have no direct CLI equivalent --> | Use DB/AST-backed data. Do not infer runtime profile from filenames alone when artifact metadata exists. |
