...
8. After the exported project is validated and the pipeline fix is complete, persist the corrected module classification with `reverts-cli module-classify --input <db> --project-id <id> --batch <TSV> --apply` (and re-run `reverts-cli match-packages --input <db> --project-id <id> --apply` to refresh the package mapping). Do not leave corrected package relationships only in generated output or transient analysis state.
...
| **D. Package-attribution defects** | TS2305 (`Module 'X' has no exported member 'Y'`), TS2307 (`Cannot find module 'X'`) | Wrong `cat=pkg` / `pkg=...` / `ver=...` on a module, or import bound to the wrong package version | ReverTS pipeline (classification / version resolution). Use [decompile](../decompile/SKILL.md) Phase 5.1c misclassification scan, then correct the attribution with `reverts-cli module-classify … --apply` + `reverts-cli match-packages … --apply` and regenerate. |
